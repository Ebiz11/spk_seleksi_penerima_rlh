...
<div class="grafik" style="min-width: 400px; height: 400px; margin: 0 auto"></div>
        <?php
        require ("library.php");
        ...
        $array_series = array();
        $array_datas = array();
        $array_attribute = array('Diterima', 'Ditolak');
        ...
        $hasil=$individu->hasil_chart_seleksi($values);
        while ($data_hasil=$hasil->fetch(PDO::FETCH_ASSOC)) {
        $diterima = $data_hasil['diterima'];
        $ditolak = $data_hasil['ditolak'];
        $array_datas[$val['periode']]['Diterima'] = intval($diterima);
        $array_datas[$val['periode']]['Ditolak'] = intval($ditolak);
        }
        }
